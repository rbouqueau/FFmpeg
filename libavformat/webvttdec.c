...
static int webvtt_read_header(AVFormatContext *s)
{
    ...
    for (;;) {
        ...
        if (!strncmp(p, "STYLE", 5)) {
            const size_t styling_entry_len = strlen(p) + 1;
            if (!styling) {
                styling = av_strdup(p);
                styling_len = styling_entry_len;
            } else {
                styling = av_realloc(styling, styling_len + 1 + styling_entry_len);
                styling[styling_len - 1] = '\n'; //replace \0 by EOL separator
                styling[styling_len] = '\n'; //add another separator
                memcpy(styling + styling_len + 1, p, styling_entry_len); //concatenate styles
                styling_len += styling_entry_len + 1;
            }
